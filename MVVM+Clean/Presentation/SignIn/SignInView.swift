...
struct SignInView: View {
    ...
    var body: some View {
        HStack {
            if self.showCard {
                CardView(color: Color(UIColor.systemBackground), cornerRadius: .large) {
                    VStack {
                        VStack(alignment: .leading) {
                            ...
                            TextField("User Name", text: self.$viewModel.username, prompt: nil)
                                ...
                                .padding()
                                .background(RoundedRectangle(cornerRadius: 8.0).stroke(Color(UIColor.systemGray)))
...
//    Group {
//      SignInView(isPresented: true, viewModel: SignInViewModel())
